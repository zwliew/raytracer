...
public:
  ...
  [[nodiscard]] constexpr bool is_vector() const { return equal(w, 0.0f); }
...
template <class T> [[nodiscard]] constexpr Tuple<T> operator-(Tuple<T> tuple) {
  tuple *= -1;
  return tuple;
}
...
template <class T> [[nodiscard]] constexpr T abs(const Tuple<T> &tuple) {
...
template <class T>
[[nodiscard]] constexpr T inner_product(const Tuple<T> &lhs,
                                        const Tuple<T> &rhs) {
  return lhs.x * rhs.x + lhs.y * rhs.y + lhs.z * rhs.z + lhs.w * rhs.w;
}

template <class T>
[[nodiscard]] constexpr Tuple<T> cross_product(const Tuple<T> &lhs,
                                               const Tuple<T> &rhs) {
